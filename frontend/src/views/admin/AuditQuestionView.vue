    ...
    <div v-else class="grid gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-3" v-loading="loading">
      <div v-for="q in list" :key="q.questionID" class="card p-4 flex flex-col gap-3">
        ...
        <div class="grid grid-cols-2 gap-2 pt-2 border-t border-gray-100 mt-auto">
          ...
          <button v-if="tab === 'approved'" class="col-span-2 inline-flex items-center justify-center gap-1.5 rounded-md bg-red-50 border border-red-200 px-3 py-2 text-xs font-medium text-red-600 hover:bg-red-100 transition" @click="adminDelete(q.questionID)">
