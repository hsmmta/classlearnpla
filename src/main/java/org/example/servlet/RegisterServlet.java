...
public class RegisterServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            String sql = "INSERT INTO user (userphone, userPassword, userName, classID, studentID, gender, userEmail, userStatus) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?, 1)";
            ...
            if (rows > 0) {
                // 为新用户创建积分记录（默认为0分）
                try {
                    String pointsSql = "INSERT INTO points (userID, userName, points) VALUES (?, ?, 0)";
                    PreparedStatement pointsPstmt = conn.prepareStatement(pointsSql);
                    pointsPstmt.setString(1, userID);
                    pointsPstmt.setString(2, userName);
                    pointsPstmt.executeUpdate();
                    pointsPstmt.close();
                } catch (Exception e) {
                    // 如果积分表不存在或出错，不影响用户注册
                    System.out.println("创建积分记录失败：" + e.getMessage());
                }
