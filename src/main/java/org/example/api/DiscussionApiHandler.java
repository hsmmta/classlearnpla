...
public class DiscussionApiHandler {
    ...
    private void deleteOwnComment(String qid, String commentId, HttpServletRequest request, HttpServletResponse response) throws SQLException, IOException {
        ...
        try {
            ...
            Integer parentCommentID = null;
            try (PreparedStatement ps = conn.prepareStatement(
                    "SELECT userID, parentCommentID FROM question_comment WHERE questionID = ? AND commentID = ? FOR UPDATE")) {
                ps.setString(1, qid);
                ps.setInt(2, cid);
                ResultSet rs = ps.executeQuery();
                if (rs.next()) {
            ...
            if (isRoot) {
                ...
                try (PreparedStatement ps = conn.prepareStatement(
                        "DELETE FROM question_comment WHERE questionID = ? AND commentID = ? AND userID = ?")) {
                    ps.setString(1, qid);
                    ps.setInt(2, cid);
                    ps.setString(3, userID);
                    int n = ps.executeUpdate();
