...
public class LoginServlet extends HttpServlet {
    ...
    private String getUserType(String userID) {
        ...
        try {
            ...
            // 假设user表中有userType字段，值为"admin"或"user"
    ...
    private String checkUserStatus(String userID) {
        ...
        try {
            conn = DBUtil.getConnection();
            String sql = "SELECT userStatus FROM user WHERE userID = ?";
            pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, userID);
            rs = pstmt.executeQuery();
            if (rs.next()) {
    ...
    private boolean verifyPassword(String userID, String userPassword, HttpSession session) {
        ...
        try {
            conn = DBUtil.getConnection();
            String sql = "SELECT userName, userPassword, classID, gender, studentID, userEmail, userType FROM user WHERE userID = ? AND userStatus = 'activate'";
            pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, userID);
            rs = pstmt.executeQuery();
            if (rs.next()) {
                ...
                if (dbPassword.equals(userPassword)) {
                    ...
                    session.setAttribute("userType", userType); // 存储用户类型到Session
                    return true;
                } else {
                    return false;
                }
    ...
    private boolean fillUserInfoToSession(String userID, HttpSession session) {
        ...
        try {
            conn = DBUtil.getConnection();
            String sql = "SELECT userName, classID, gender, studentID, userEmail, userType FROM user WHERE userID = ? AND userStatus = 'activate'";
            pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, userID);
            rs = pstmt.executeQuery();
            if (rs.next()) {
                ...
                session.setAttribute("userType", rs.getString("userType")); // 存储用户类型到Session
